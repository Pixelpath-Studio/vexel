// Public types for the TraceView library.
//
// This local copy mirrors what `@trace/runtime` will export from
...
export type TraceSource = string | Uint8Array | RemoteSource;
...
export class TraceError extends Error {
  readonly kind:
    ...
    | 'hash-mismatch'
    | 'invalid-source';
  constructor(kind: TraceError['kind'], message: string) {
    super(message);
    this.kind = kind;
...
export type TraceDecorator = (ctx: DecoratorContext) => ReactNode;

export interface TracePluginAPI {
  registerDecorator(decorator: TraceDecorator): void;
...
export interface TracePlugin {
  name: string;
  install(api: TracePluginAPI): void | (() => void);
}
...
export interface TraceViewProps {
  ...
  errorFallback?: ReactNode | ((error: TraceError) => ReactNode);

  // -------- EXTENSIBILITY --------
  decorators?: TraceDecorator[];
  plugins?: TracePlugin[];
