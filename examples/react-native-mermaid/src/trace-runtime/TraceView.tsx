// <TraceView> — the public component.
//
...
export function TraceView(props: TraceViewProps) {
  ...
  const [pluginDecorators, setPluginDecorators] = useState<TraceDecorator[]>([]);
  ...
  useEffect(() => {
    ...
    const collected: TraceDecorator[] = [];
    pluginResolversRef.current = new Map();
    pluginCommandsRef.current = new Map();
    const teardowns: Array<void | (() => void)> = [];
    const api: TracePluginAPI = {
...
function DefaultErrorFallback({ error }: { error: TraceError }) {
  return (
...
function sourceKey(source: TraceSource): string {
...
export type { Graph, SelectionState, HighlightMode, Easing, TracePlugin, TraceDecorator } from './types';
