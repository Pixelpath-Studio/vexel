...
pub const HEADER_MAGIC: &[u8; 4] = b"VEXL";
pub const FOOTER_MAGIC: &[u8; 4] = b"vex!";
...
impl Header {
    ...
    pub fn parse(bytes: &[u8]) -> Result<Self> {
        if bytes.len() < HEADER_SIZE {
            return Err(VexelError::InvalidFile("file shorter than header"));
        }
        if &bytes[0..4] != HEADER_MAGIC {
            return Err(VexelError::InvalidFile("bad header magic (expected TRCE)"));
        }
...
impl Footer {
    ...
    pub fn parse(bytes: &[u8]) -> Result<Self> {
        if bytes.len() < FOOTER_SIZE {
            return Err(VexelError::InvalidFile("file shorter than footer"));
        }
        if &bytes[8..12] != FOOTER_MAGIC {
            return Err(VexelError::InvalidFile("bad footer magic (expected trc!)"));
        }
        Ok(Self {
