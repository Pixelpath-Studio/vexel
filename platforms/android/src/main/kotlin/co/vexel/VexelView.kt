...
/** Android counterpart of VexelView (SPEC §6.2). */
class VexelView @JvmOverloads constructor(
    ...
    private var traceFile: VexelFile? = null
    private var traceSession: VexelSession? = null
        ...
        try {
            when (src) {
                is Source.File -> {
                    ...
                    val f = VexelFile(bytes)
                    renderer.load(f)
                    traceFile = f; traceSession = null
                }
                is Source.Bytes -> {
                    val f = VexelFile(src.data)
                    renderer.load(f)
                    traceFile = f; traceSession = null
                }
                is Source.Streaming -> {
                    traceSession = src.session
                    val f = VexelFile(src.session.snapshot())
                    renderer.load(f)
                    traceFile = f
                }
            }
            ...
            android.util.Log.e("Vexel", "load failed", e)
        }
    }
