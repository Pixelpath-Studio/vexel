// Fabric component implementation. Hosts a `Vexel.VexelView` (from
// Vexel.framework) inside a UIView managed by the RN view registry.
//
...
@objc(RNVexelViewManager)
final class RNVexelViewManager: RCTViewManager {
    override class func requiresMainQueueSetup() -> Bool { true }
    override func view() -> UIView! { RNVexelViewWrapper(frame: .zero) }
}

final class RNVexelViewWrapper: UIView, VexelViewDelegate {
    ...
    private func reload() {
        if sessionHandle.intValue != 0 {
            ...
            // Vexel.framework Animator, which already supports session input.
