// TraceView — React Native component. Wraps a Fabric native view that renders
// the .trace source via Skia (provided by @shopify/react-native-skia).
//
...
import RNTraceView from './native/TraceViewNativeComponent';
import NativeTrace from './native/NativeTrace';
import type { TraceSession, TraceViewProps } from './types';
...
function isSession(s: TraceViewProps['source']): s is TraceSession {
  return typeof s === 'object' && s !== null && typeof (s as TraceSession).append === 'function';
}
...
// useTraceSession; we expose a getter via a non-enumerable property to avoid
// polluting the public TraceSession type.
const SESSION_HANDLE_KEY = '__traceSessionHandle';
function getSessionHandle(s: TraceSession): number {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
