...
import type { FragmentAnim, VexelSession, ViewBox, Backpressure } from './types';

interface UseVexelSessionOptions {
  viewBox: ViewBox;
...
/**
 * React hook that returns a VexelSession backed by the Rust core's `Session`.
 *
 ...
 * API, and (c) notify subscribers (VexelView) on every mutation so the view
 * re-snapshots.
 *
 ...
 */
export function useVexelSession(opts: UseVexelSessionOptions): VexelSession {
  const handleRef = useRef<number | null>(null);
  const listenersRef = useRef<Set<() => void>>(new Set());

  if (handleRef.current == null) {
    if (!NativeTrace) {
      throw new Error('@pixelpath/vexel: native module not registered.');
    }
