...
export interface Spec extends TurboModule {
  /** Synchronous SVG → .vex bytes. Returns the bytes as base64 (codegen has
   * no Uint8Array support yet; the JSI direct binding bypasses this). */
  convert(svg: string, generator: string | null): string;

  /** Inspect a .vex file without instantiating a view. Returns JSON. */
  ...
  sessionSnapshot(sessionHandle: number): string; // base64 .vex bytes
