//! Regenerate every conformance fixture's `output.trace` from its `input.svg`
//! or `input.json`. Run from the repo root:
//!
//!   cargo run -p trace-core --example gen_fixtures
//!
...
use trace_core::{convert_svg_to_trace, ConvertOptions, Ir, ViewBox};
...
fn main() -> std::io::Result<()> {
    ...
    for entry in entries {
        ...
        let out_path = dir.join("output.trace");
...
fn build_from_svg(dir: &Path) -> Option<Vec<u8>> {
    ...
    convert_svg_to_trace(&svg, &opts).ok()
}
