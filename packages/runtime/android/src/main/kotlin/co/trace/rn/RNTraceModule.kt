...
import co.trace.Easing
import co.trace.FragmentAnim
import co.trace.StartAfter
import co.trace.TraceFile
import co.trace.TraceSession
import co.trace.generated.convertSvgToTrace
import co.trace.generated.ConvertOptions
...
/** TurboModule. Bridges JS calls to the Rust core via the trace-android AAR. */
...
class RNTraceModule(context: ReactApplicationContext) : ReactContextBaseJavaModule(context) {
    ...
    private val sessions = mutableMapOf<Int, TraceSession>()
    ...
    fun createSession(x: Double, y: Double, w: Double, h: Double): Int {
        val s = TraceSession(RectF(x.toFloat(), y.toFloat(), (x + w).toFloat(), (y + h).toFloat()))
        val h = nextHandle++
