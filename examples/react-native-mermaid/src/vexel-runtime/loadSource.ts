// Source loader. Resolves any `VexelSource` (string / Uint8Array / RemoteSource)
// to a raw SVG string. Async so the consumer can render a placeholder while
// the request is in flight.

import { VexelError, type VexelSource } from './types';

export async function loadSource(source: VexelSource): Promise<string> {
  if (typeof source === 'string') return source;
  if (source instanceof Uint8Array) {
    try {
      return new TextDecoder('utf-8').decode(source);
    } catch (e: any) {
      throw new VexelError('invalid-source', `Uint8Array decode failed: ${e.message}`);
    }
  }
  if (source && typeof source === 'object' && 'uri' in source) {
    return await loadRemote(source.uri, source);
  }
  throw new VexelError('invalid-source', 'Unknown source type');
}
...
async function loadRemote(
  ...
  try {
    ...
    throw new VexelError('load-failed', `fetch ${uri}: ${e.message}`);
  }
  if (!res.ok) {
    throw new VexelError('load-failed', `HTTP ${res.status} fetching ${uri}`);
  }
