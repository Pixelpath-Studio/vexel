...
impl<'a> Geom<'a> {
    pub fn new(bytes: &'a [u8]) -> Result<Self> {
        if bytes.len() < 4 {
            return Err(VexelError::InvalidFile("GEOM section too small"));
        }
...
impl<'a> Iterator for GeomIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if start + ELEMENT_HEADER_SIZE > self.bytes.len() {
            return Some(Err(VexelError::InvalidFile(
        ...
        if cursor + verb_count > self.bytes.len() {
            return Some(Err(VexelError::InvalidFile("GEOM verbs truncated")));
        }
        ...
        if cursor + points_byte_len > self.bytes.len() {
            return Some(Err(VexelError::InvalidFile("GEOM points truncated")));
        }
...
impl<'a> ElementView<'a> {
    ...
    pub fn to_path(&self) -> Result<Path> {
        ...
        for &v in self.verbs {
            verbs.push(Verb::from_u8(v).ok_or(VexelError::InvalidFile("unknown path verb"))?);
        }
