...
impl<'a> Strs<'a> {
    pub fn new(bytes: &'a [u8]) -> Result<Self> {
        if bytes.is_empty() {
            return Err(VexelError::InvalidFile("STRS section is empty"));
        }
        if bytes[0] != 0 {
    ...
    pub fn get(&self, offset: u32) -> Result<&'a str> {
        ...
        if off >= self.bytes.len() {
            return Err(VexelError::InvalidFile("STRS offset out of bounds"));
        }
        let end = self.bytes[off..]
            .iter()
            .position(|&b| b == 0)
            .ok_or(VexelError::InvalidFile("unterminated STRS entry"))?;
        std::str::from_utf8(&self.bytes[off..off + end])
            .map_err(|_| VexelError::InvalidFile("STRS entry is not UTF-8"))
    }
}
