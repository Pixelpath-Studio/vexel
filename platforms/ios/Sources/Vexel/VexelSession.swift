// VexelSession.swift — streaming session for AI-drawn content. Wraps the Rust
// core's `Session` via UniFFI. Mirrors the API in SPEC §4.5.

import Foundation

public final class VexelSession {
    fileprivate let inner: VexelCore.Session

    public init(viewBox: CGRect) {
        self.inner = VexelCore.Session(viewbox: VexelCore.ViewBox(
...
public struct FragmentAnim {
    ...
    fileprivate func toCore() -> VexelCore.FragmentAnim {
        VexelCore.FragmentAnim(
...
public enum StartAfter {
    ...
    fileprivate func toCore() -> VexelCore.StartAfter {
        switch self {
...
public enum Easing {
    ...
    fileprivate func toCore() -> VexelCore.Easing {
        switch self {
