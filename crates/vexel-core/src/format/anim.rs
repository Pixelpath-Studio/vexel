...
impl<'a> Anim<'a> {
    pub fn new(bytes: &'a [u8]) -> Result<Self> {
        if bytes.len() < 4 {
            return Err(VexelError::InvalidFile("ANIM header truncated"));
        }
        Ok(Self {
            bytes,
...
impl<'a> Iterator for AnimIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if start + TRACK_HEADER_SIZE > self.bytes.len() {
            return Some(Err(VexelError::InvalidFile("ANIM track header truncated")));
        }
        ...
        let track_type = match TrackType::from_u8(b[4]) {
            Some(t) => t,
            None => return Some(Err(VexelError::InvalidFile("unknown ANIM track type"))),
        };
        let start_ms = LE::read_u32(&b[5..9]);
        let duration_ms = LE::read_u32(&b[9..13]);
        let easing = match Easing::from_u8(b[13]) {
            Some(e) => e,
            None => return Some(Err(VexelError::InvalidFile("unknown ANIM easing"))),
        };
        ...
        if payload_end > self.bytes.len() {
            return Some(Err(VexelError::InvalidFile("ANIM payload truncated")));
        }
