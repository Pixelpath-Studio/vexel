...
use trace_core::{convert_svg_to_trace, ConvertOptions, TraceFile};
...
fn convert_simple_svg() {
    ...
    let bytes = convert_svg_to_trace(SIMPLE, &opts).expect("convert");
    ...
    assert_ne!(dot.fill, trace_core::Rgba::TRANSPARENT);
    assert_ne!(dot.stroke, trace_core::Rgba::TRANSPARENT);
...
fn mermaid_id_normalization() {
    ...
    let bytes = convert_svg_to_trace(svg, &ConvertOptions::default()).unwrap();
...
fn deterministic_conversion() {
    let a = convert_svg_to_trace(SIMPLE, &ConvertOptions::default()).unwrap();
    let b = convert_svg_to_trace(SIMPLE, &ConvertOptions::default()).unwrap();
    assert_eq!(
        a, b,
