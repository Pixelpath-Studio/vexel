...
// to the Rust core via the Vexel.framework Swift API.

import Foundation
import React  // RCTEventEmitter, RCTBridgeModule
import Vexel  // our SwiftPM/CocoaPods framework
...
final class RNTraceModule: NSObject, RCTBridgeModule {
    ...
    private static var sessions: [Int: VexelSession] = [:]
    private static var nextHandle: Int = 1

    @objc
    func convert(_ svg: String, generator: String?) -> String {
        do {
            let bytes = try VexelCore.convertSvgToTrace(
                svg: svg,
                options: VexelCore.ConvertOptions(
                    generator: generator,
    ...
    func inspect(_ b64: String) -> String {
        guard let data = Data(base64Encoded: b64), let file = try? VexelFile(data: data) else {
            return "{\"error\":\"parse\"}"
        }
        let vb = file.viewBox
        let ids = file.ids
    ...
    func createSession(_ x: NSNumber, y: NSNumber, w: NSNumber, h: NSNumber) -> NSNumber {
        ...
        let session = VexelSession(viewBox: vb)
    ...
    private static func register(_ session: VexelSession) -> Int {
        sessionsLock.lock(); defer { sessionsLock.unlock() }
        let h = nextHandle; nextHandle += 1
        sessions[h] = session
        return h
    }
    private static func session(for handle: Int) -> VexelSession? {
        sessionsLock.lock(); defer { sessionsLock.unlock() }
