//! Zero-copy reader over a `.trace` byte buffer. Parses the header, walks the
//! section table, and exposes accessors per section. Sections themselves are
//! parsed lazily by the higher-level `TraceFile` in `crate::api`.
...
impl<'a> FileReader<'a> {
    pub fn parse(bytes: &'a [u8]) -> Result<Self> {
        if bytes.len() < HEADER_SIZE + FOOTER_SIZE {
            return Err(TraceError::InvalidFile("file too small"));
        }
        ...
        if expected_crc != footer.body_crc32 {
            return Err(TraceError::InvalidFile("body CRC32 mismatch"));
        }
        ...
        if table_end > body_end {
            return Err(TraceError::InvalidFile("section table runs past file end"));
        }
        ...
        for i in 0..(header.section_count as usize) {
            ...
            let payload_end = (entry.offset as usize)
                ...
                .ok_or(TraceError::InvalidFile("section size overflow"))?;
