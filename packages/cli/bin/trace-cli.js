#!/usr/bin/env node
// @trace/cli — npm wrapper that dispatches to the native trace-cli binary.
//
// At publish time, this package downloads the matching trace-cli binary for the
...
// it under node_modules/@trace/cli/bin/native/. Modeled after esbuild's
...
// from source via `cargo run -p trace-cli`.
...
function resolveNativeBinary() {
  const exe = process.platform === 'win32' ? 'trace-cli.exe' : 'trace-cli';
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (fs.existsSync(path.join(dir, 'Cargo.toml'))) {
      return {
        mode: 'cargo',
        cmd: 'cargo',
        args: ['run', '--quiet', '-p', 'trace-cli', '--release', '--'],
        cwd: dir,
      };
    }
  }
  console.error(
    '@trace/cli: no native binary found and no Cargo workspace nearby. ' +
    'Reinstall the package or build from source with `cargo install trace-cli`.'
  );
  process.exit(127);
}
...
if (result.error) {
  console.error('@trace/cli:', result.error.message);
  process.exit(1);
}
