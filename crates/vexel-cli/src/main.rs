//! `vexel-cli` — reference CLI for the Vexel format.
//!
//! Subcommands per SPEC §10:
//!   convert <input.svg> [--out <output.vex>]
//!   inspect <file.vex>
//!   dump    <file.vex>
//!   validate <file.vex>
//!   diff    <a.vex> <b.vex>
//!   pack    <input.json> [--out <output.vex>]   (round-trips dump output)
//!
...
use vexel_core::{ConvertOptions, VexelFile};
...
fn main() -> ExitCode {
    ...
    match sub.as_str() {
        ...
        "--version" | "-v" => {
            println!("vexel-cli {}", env!("CARGO_PKG_VERSION"));
            ExitCode::SUCCESS
        }
        other => {
            eprintln!("vexel-cli: unknown subcommand: {other}");
            print_usage();
            ExitCode::from(2)
        }
    }
...
fn print_usage() {
    eprintln!(
        "vexel-cli {version} — reference CLI for the Vexel format

USAGE:
  vexel-cli convert  <input.svg>     [--out <output.vex>] [--generator <name>]
  vexel-cli inspect  <file.vex>
  vexel-cli dump     <file.vex>
  vexel-cli validate <file.vex>
  vexel-cli diff     <a.vex> <b.vex>
  vexel-cli pack     <input.json>    [--out <output.vex>]

SUBCOMMANDS:
  convert    SVG → .vex. Defaults output path to <input>.vex.
  ...
  diff       Structural diff of two .vex files.
  pack       JSON → .vex (reverse of `dump`).
",
...
fn convert(args: &[String]) -> Result<(), String> {
    ...
    let opts = ConvertOptions {
        generator: generator.or_else(|| Some(format!("vexel-cli {}", env!("CARGO_PKG_VERSION")))),
        ..ConvertOptions::default()
    };
    let bytes = vexel_core::convert_svg_to_vex(&svg, &opts).map_err(|e| format!("convert: {e}"))?;
...
fn inspect(args: &[String]) -> Result<(), String> {
    let path = args.first().ok_or("missing <file.vex>")?;
    let bytes = fs::read(path).map_err(|e| format!("reading {path}: {e}"))?;
    let file = VexelFile::parse(&bytes).map_err(|e| format!("parse: {e}"))?;
    println!("file:         {path}");
...
fn validate(args: &[String]) -> Result<(), String> {
    let path = args.first().ok_or("missing <file.vex>")?;
    let bytes = fs::read(path).map_err(|e| format!("reading {path}: {e}"))?;
    let file = VexelFile::parse(&bytes).map_err(|e| format!("invalid: {e}"))?;
...
fn pack(args: &[String]) -> Result<(), String> {
    ...
    let mut ir = vexel_core::Ir::new(vb);
    for (k, v) in metadata {
        ir.set_metadata(k, v);
    }
    let bytes = vexel_core::write(&ir).map_err(|e| format!("serialize: {e}"))?;
    let out_path = output.unwrap_or_else(|| {
        let mut p = input.clone();
        p.set_extension("trace");
        p
...
fn parse_pack_json(json: &str) -> Result<(vexel_core::ViewBox, Vec<(String, String)>), String> {
    ...
    let vb = vexel_core::ViewBox::new(nums[0], nums[1], nums[2], nums[3]);
...
fn diff(args: &[String]) -> Result<(), String> {
    if args.len() < 2 {
        return Err("missing <a.vex> <b.vex>".into());
    }
    ...
    let a = VexelFile::parse(&a_bytes).map_err(|e| format!("parse {}: {e}", args[0]))?;
    let b = VexelFile::parse(&b_bytes).map_err(|e| format!("parse {}: {e}", args[1]))?;
    let mut any = false;
