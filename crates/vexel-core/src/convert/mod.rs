...
/// Options for the SVG → .vex converter. Mirrors the eventual UniFFI surface.
#[derive(Debug, Clone)]
...
pub fn convert_svg_to_ir(svg: &str, opts: &ConvertOptions) -> Result<Ir> {
    ...
    let tree = Tree::from_str(svg, &usvg_opts).map_err(|e| VexelError::SvgParse(e.to_string()))?;
...
/// Top-level conversion, returning serialized `.vex` bytes (includes HITX
/// so the file supports hit-testing out of the box).
pub fn convert_svg_to_vex(svg: &str, opts: &ConvertOptions) -> Result<Vec<u8>> {
...
fn walk_group(group: &Group, ir: &mut Ir, budget: &mut ElementBudget) -> Result<()> {
    if budget.depth >= budget.max_depth {
        return Err(VexelError::InvalidFile("nesting exceeds 1024 depth"));
    }
    budget.depth += 1;
    for node in group.children() {
        if budget.remaining == 0 {
            return Err(VexelError::LimitExceeded("element count over 1M"));
        }
        match node {
