...
fn empty_file_roundtrip() {
    ...
    // Public TraceFile API.
    let file = TraceFile::parse(&bytes).expect("trace file parse");
    let vb = file.viewbox();
...
fn deterministic_serialization() {
    ...
    // foundation of the conformance suite (canonical `output.trace` per fixture).
    let ir = Ir::new(ViewBox::new(0.0, 0.0, 100.0, 100.0));
    let a = trace_core::write(&ir).unwrap();
    let b = trace_core::write(&ir).unwrap();
...
fn rejects_bad_magic() {
    let mut bytes = trace_core::write(&Ir::new(ViewBox::new(0.0, 0.0, 1.0, 1.0))).unwrap();
    bytes[0] = b'X';
    let err = TraceFile::parse(&bytes).unwrap_err();
    assert!(matches!(err, trace_core::TraceError::InvalidFile(_)));
}
