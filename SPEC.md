# Vexel — Final Specification and Technical Architecture (v1.0)

**An open-source vector graphics format and rendering runtime for mobile.**

Vexel is a binary file format (`.vex`) and a cross-platform plugin that renders it natively on iOS and Android with pixel-identical output, powered by Skia. It is designed to replace SVG as the *rendering* format for mobile apps — for static illustrations, pre-generated diagrams (Mermaid, org charts), and AI-authored, progressively-drawn content (like a teacher writing on a whiteboard).

You install one npm package. You convert any SVG to `.vex` (at build time or at runtime). You drop a `<VexelView>` component into your app. It renders fast, identically on both platforms, with per-element interactivity. That's the entire product surface.
...
```
                  ...
                  ┌──────────────────────────────────────────┐
                  │  @pixelpath/vexel (npm)                    │
                  │  - <VexelView /> React Native component  │
                  ...
                  ▼                                          ▼
         ┌──────────────────┐                    ┌──────────────────────┐
         │  Vexel.framework │                    │  vexel-android.aar   │
...
**License:** Apache 2.0 · **Working name:** Vexel · **File extension:** `.vex` · **MIME:** `application/vnd.vexel+binary` · **Reference consumer:** Curo

---

## Implementation notes for Claude Code (read first)

**Identity preservation requirement (critical).** The runtime must maintain a one-to-one mapping from element index (in GEOM order) to a drawable handle, such that any element can be independently re-styled, animated, or highlighted at any time. SkPicture caching is permitted **only** for elements with `flags.is_decorative == true`; all other elements must be drawn via individual `canvas.drawPath` calls with their own paint state. Hit-testing routes through `vexel_core::hit_test` (Rust); the platform runtime never queries Skia for hit-testing because Skia has no concept of element identity.
...
3. [The `.vex` binary format](#3-the-trace-binary-format)
4. [The Rust core (`vexel-core`)](#4-the-rust-core-vexel-core)
5. [The iOS runtime](#5-the-ios-runtime)
6. [The Android runtime](#6-the-android-runtime)
7. [The npm package (`@pixelpath/vexel`)](#7-the-npm-package-tracenuntime)
8. [The streaming protocol](#8-the-streaming-protocol)
9. [The drawing-motion model](#9-the-drawing-motion-model)
10. [CLI (`@pixelpath/vexel-cli`)](#10-cli-tracecli)
...
Vexel is structured as **one organization, four published artifacts**, with one open Rust monorepo behind them.

| Artifact | Distribution | Audience |
|---|---|---|
| `@pixelpath/vexel` | npm (peer dep: `@shopify/react-native-skia`) | React Native developers (primary) |
| `@pixelpath/vexel-cli` | npm (`npx @pixelpath/vexel-cli ...`) | Build pipelines, anyone with an SVG to convert |
| `Vexel.framework` | SwiftPM + CocoaPods (depends on Skia.framework via RN-Skia's distribution) | Native iOS developers (no React Native) |
| `co.vexel:vexel-android` | Maven Central (depends on Skia .so via RN-Skia's distribution) | Native Android developers (no React Native) |

The Rust core (`vexel-core`) is the engine behind all of them. Open-source, but never the user-facing artifact.
...
## 3. The `.vex` binary format

### 3.1 Design principles

1. Binary, little-endian, 4-byte aligned. JSON never on a hot path. `.vex` is `mmap`-readable.
...
## 4. The Rust core (`vexel-core`)

The Rust core does **everything algorithmic**: parsing SVG, normalizing, baking transforms, flattening to polylines, building the R-tree, serializing to `.vex`, providing the query API, managing streaming sessions. It contains **no platform graphics code**. It does not depend on Skia, CoreGraphics, or Canvas. Platforms only handle drawing.
...
```rust
// Conversion
pub fn convert_svg_to_vex(svg: &str, options: ConvertOptions) -> Result<Vec<u8>, VexelError>;

// Batch query
pub struct VexelFile<'a> { /* zero-copy view over &[u8] */ }

impl<'a> VexelFile<'a> {
    pub fn parse(bytes: &'a [u8]) -> Result<Self, VexelError>;
...
`snapshot` produces a complete `.vex` (with `IS_STREAMING_SNAPSHOT` set). `delta_since(v)` produces a minimal delta with only elements added/changed since `v`. The platform layer applies the delta by appending GEOM entries, merging IDIX, and scheduling new animations.
...
├── Vexel.podspec                # CocoaPods
├── Sources/Vexel/
│   ├── Vexel.swift              # umbrella
│   ├── VexelFile.swift          # wraps Rust VexelFile via UniFFI
│   ├── VexelSession.swift       # wraps Rust Session
│   ├── VexelView.swift          # UIView subclass, hosts a Skia surface
...
└── VexelCore.xcframework        # Rust .a bundled via uniffi-bindgen-swift
```

### 5.2 The `VexelView` API
...
The `VexelView` hosts a Skia drawing surface (provided by the Skia framework that `react-native-skia` distributes, or by linking Skia directly for the standalone iOS framework). The rendering loop:

1. On load: parse the `.vex` file via Rust. For each element in GEOM, build an `SkPath` from the verb stream and store it in a map `Map<element_index, ElementHandle>`, where `ElementHandle` holds the SkPath, the current Paint, and per-element animation state.
...
│   │   ├── VexelFile.kt
│   │   ├── VexelSession.kt
│   │   ├── VexelView.kt            # View subclass, hosts Skia surface
...
│   └── jniLibs/
│       ├── arm64-v8a/libvexel_core.so
│       ├── armeabi-v7a/libvexel_core.so
│       ├── x86_64/libvexel_core.so
│       └── x86/libvexel_core.so
```

### 6.2 The `VexelView` API

```kotlin
class VexelView @JvmOverloads constructor(
    context: Context,
    ...
    sealed class Source {
        ...
        data class Streaming(val session: VexelSession) : Source()
    }
...
1. On load: parse `.vex`, build SkPath per element via JNI calls to Rust, store in `Map<Int, ElementHandle>`.
...
## 7. The npm package (`@pixelpath/vexel`)
...
├── src/
│   ├── index.ts                  # public API
│   ├── VexelView.tsx             # Fabric component, wraps RN-Skia canvas
│   ├── useVexelSession.ts        # streaming hook
│   ├── convert.ts                # SVG → Uint8Array (calls Rust via TurboModule)
│   ├── types.ts
│   └── native/
│       ├── NativeTrace.ts        # TurboModule spec (codegen input)
│       ├── VexelViewNativeComponent.ts  # Fabric spec
│       └── jsi.ts                # direct JSI bindings (advanced)
├── ios/
│   ├── Vexel.podspec → links Vexel.framework
│   ├── RNVexelView.swift         # Fabric component impl
│   └── RNTraceModule.swift       # TurboModule impl
├── android/
│   ├── build.gradle → links co.vexel:vexel-android
│   ├── src/main/kotlin/.../RNVexelViewManager.kt
...
// Batch render
export const VexelView: React.FC<{
...
// Streaming
export function useVexelSession(opts: {
...
- **Fabric component** (`VexelViewNativeComponent`) for the rendering surface, wrapping a `react-native-skia` Canvas internally.
...
- Rust UDL in `vexel-core/src/api/api.udl` → TypeScript types and TurboModule + JSI C++ glue
...
`@pixelpath/vexel` declares `@shopify/react-native-skia` as a peer dependency. The runtime uses RN-Skia's `Canvas` component internally as the drawing surface. This means:

- Users install both packages.
- The Skia binaries come from RN-Skia (not bundled by Vexel), avoiding duplication.
- Vexel inherits RN-Skia's React Native version support matrix.
- Total binary added by Vexel on top of RN-Skia: only the Rust core (~2-3 MB per ABI).
...
Vexel is **transport-agnostic**. WebSocket, SSE, polling — application's choice. What gets transmitted is a sequence of **fragment messages**:
...
Set per session: `useVexelSession({ viewBox: ..., backpressure: 'catchUp' });`

---

## 9. The drawing-motion model

This is what makes Vexel look like a teacher drawing rather than a robot revealing.
...
## 10. CLI (`@pixelpath/vexel-cli`)

```bash
# Convert one or more SVGs to .vex
npx @pixelpath/vexel-cli convert input.svg --out output.vex
npx @pixelpath/vexel-cli convert "icons/*.svg" --out-dir assets/

# Inspect a .vex file
npx @pixelpath/vexel-cli inspect file.vex
# → viewBox, element count, ids, metadata, animation tracks

# Dump a .vex file as JSON (for debugging)
npx @pixelpath/vexel-cli dump file.vex --pretty
# Every byte of a .vex file is round-trippable through JSON; dump produces
# editable JSON, and `npx @pixelpath/vexel-cli pack input.json` reconstructs the .vex.
# This is the debuggability escape hatch.

# Validate
npx @pixelpath/vexel-cli validate file.vex

# Diff two .vex files
npx @pixelpath/vexel-cli diff a.vex b.vex
```
...
Shipped as a separate package `@pixelpath/vexel-conformance` to make the format real (not "whatever the Rust code does"). Lottie has one; Rive has one. Vexel must have one from day one.
...
├── fixtures/
│   ├── 001-empty/              # empty .vex file
│   │   ├── input.json          # canonical JSON representation
│   │   ├── output.vex        # expected binary
...
- The expected `.vex` bytes (canonical output)
- Expected query results: viewBox, element count, all ids, hit-test results at specific (x, y), bbox of named elements

Any implementation — the reference Rust runtime now, a hypothetical Flutter or web port later — can run against this suite and prove conformance. This is how Vexel evolves from "a library" to "a format."
...
├── CONTRIBUTING.md
├── crates/
│   └── vexel-core/                 # the Rust core (§4)
├── packages/
│   ├── runtime/                    # @pixelpath/vexel (npm) (§7)
│   ├── cli/                        # @pixelpath/vexel-cli (npm) (§10)
│   ├── conformance/                # @pixelpath/vexel-conformance (npm) (§11)
│   └── docs/                       # docs site (Docusaurus/Nextra)
├── platforms/
│   ├── ios/                        # Vexel.framework (§5)
│   └── android/                    # vexel-android.aar (§6)
├── examples/
...
# → produces VexelCore.xcframework
# Vexel.framework wraps it and adds Swift API
```

UniFFI generates Swift bindings; `Vexel.framework` adds the `VexelView` UIView and the Swift idiomatic surface on top.
...
# → produces vexel-android.aar with bundled .so files
```

UniFFI generates Kotlin bindings; `vexel-android` adds the `VexelView` View and Kotlin idiomatic surface.

### 13.4 npm package distribution

`@pixelpath/vexel`'s postinstall:
...
1. Update `Cargo.toml`, `package.json`, `build.gradle`, `Vexel.podspec` to the new version (one script: `./scripts/bump-version.sh 1.2.3`).
2. Tag `v1.2.3`.
...
- `vexel-core` crate, depend on usvg
- Implement IR types
...
- Start `@pixelpath/vexel-conformance` fixture format
...
- `Vexel.framework` skeleton
- UniFFI Swift bindings generation
- `VexelFile` Swift wrapper
- `VexelView` UIView with Skia surface
- verb-stream → SkPath
...
- Static example app rendering a Mermaid `.vex`

### Week 9 · Android runtime

- `vexel-android` AAR skeleton
- UniFFI Kotlin bindings
- `VexelView` View with Skia surface
...
- `@pixelpath/vexel` npm package
- Fabric component + TurboModule wrapping RN-Skia Canvas
- `useVexelSession` hook
- Streaming example app

### Week 12 · Polish, CLI, conformance, docs, v1.0 release

- `@pixelpath/vexel-cli` package (convert, inspect, dump, validate, diff, pack)
- `@pixelpath/vexel-conformance` 200-fixture suite passing
...
After v1.0: Curo integrates `@pixelpath/vexel`, replaces existing SVG handling. First-user dogfooding informs v1.1.
...
- One screen recording with `@pixelpath/vexel` (Vexel v1.0).
...
| Should we ship a Figma plugin? | Yes, exporting nodes as `.vex` directly | v1.2 |
...
| Should there be a hosted CDN service for converting and serving `.vex`? | Maybe — only if usage justifies it. The Rust core's `convert` is fast enough that most users will convert in their build pipeline. | based on demand |
...
| Should there be a Mermaid build-time integration that emits `.vex` directly? | Yes; small Mermaid plugin that writes `.vex` instead of `.svg` | v1.1 |
