...
//
// Reads ANIM tracks from the VexelFile (or the live Session snapshot) and on
...
final class Animator {
    ...
    private let file: VexelFile
    private let session: VexelSession?
    ...
    init(renderer: TraceRenderer, file: VexelFile, session: VexelSession?,
    ...
    @objc private func tick(_ link: CADisplayLink) {
        ...
        if let session = session, session.version != lastSnapshotVersion {
            ...
            do {
                ...
                let newFile = try VexelFile(data: snap)
                ...
                NSLog("[Vexel] session refresh failed: \(error)")
            }
        }
